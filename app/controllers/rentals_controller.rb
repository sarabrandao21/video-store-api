class RentalsController < ApplicationController
  ...
  def set_customer
    ...
    if @customer.nil?
      render json: { errors: { customer_id: ["Error: couldn't find a customer with customer_id #{params[:customer_id]}"] } }, status: :not_found
    end
  end
  ...
  def set_video
    ...
    if @video.nil?
      render json: { errors: { title: ["Error: couldn't find a video with the title #{params[:title]}"] } }, status: :not_found
    end
  end
